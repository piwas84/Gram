var plugin = (function () {
    ...
    function lang () {
      Lampa.Lang.add({
        ...
        gramlink_menu_sync_on: {
          en: 'Sync: ON',
          uk: 'Синхр: УВІМК',
          ru: 'Синхр: ВКЛ'
        },
        gramlink_menu_sync_off: {
          en: 'Sync: OFF',
          uk: 'Синхр: ВИМК',
          ru: 'Синхр: ВЫКЛ'
        },
        gramlink_sync_toggled_on: {
          en: 'Synchronization enabled',
          uk: 'Синхронізацію увімкнено',
          ru: 'Синхронизация включена'
        },
        gramlink_sync_toggled_off: {
          en: 'Synchronization disabled (no network activity)',
          uk: 'Синхронізацію вимкнено (без мережевої активності)',
          ru: 'Синхронизация выключена (без сетевой активности)'
        },
    ...
    // Custom CDN URL (optional): set in Lampa Storage key "gramlink_bundle_url"
    // Example: Lampa.Storage.set('gramlink_bundle_url', 'https://your-server.com/telegram.min.js')
    var _customBundle = '';
    try {
      if (typeof Lampa !== 'undefined' && Lampa.Storage && Lampa.Storage.get) {
        _customBundle = (Lampa.Storage.get('gramlink_bundle_url', '') || '').trim();
      }
    } catch (e) {}
    var BUNDLE_URLS = [];
    if (_customBundle) BUNDLE_URLS.push(_customBundle);
    BUNDLE_URLS = BUNDLE_URLS.concat([
      'https://cdn.jsdelivr.net/gh/lampame/TGSBundle@' + BUNDLE_REF + '/telegram.min.js',
      'https://fastly.jsdelivr.net/gh/lampame/TGSBundle@' + BUNDLE_REF + '/telegram.min.js',
      'https://gcore.jsdelivr.net/gh/lampame/TGSBundle@' + BUNDLE_REF + '/telegram.min.js',
      'https://cdn.jsdmirror.com/gh/lampame/TGSBundle@' + BUNDLE_REF + '/telegram.min.js',
      'https://rawcdn.githack.com/lampame/TGSBundle/' + BUNDLE_REF + '/telegram.min.js',
      'https://raw.githack.com/lampame/TGSBundle/main/telegram.min.js',
      'https://raw.githubusercontent.com/lampame/TGSBundle/' + BUNDLE_REF + '/telegram.min.js',
      'https://cdn.jsdelivr.net/gh/lampame/TGSBundle@main/telegram.min.js'
    ]);
    var _loadPromise = null;
    function tryLoad(index) {
      if (index >= BUNDLE_URLS.length) {
        return Promise.reject(new Error('GramLink: All CDN URLs failed. Set Lampa.Storage.set(\'gramlink_bundle_url\', \'https://your-mirror/telegram.min.js\') or use VPN'));
      }
    ...
    var GramLinkClient = /*#__PURE__*/function () {
      ...
      return _createClass(GramLinkClient, [{
        ...
        value: function publishDelta(profilesSyncTopicId, subtype, profileMsgId, payload) {
          // MOD: skip all network sync when disabled
          if (!this.isEnabled('gramlink_sync_enabled')) {
        ...
        value: function publish$1(chatId, threadId, type, payload, targetDeviceId) {
          if (!this.isConnected()) return Promise.resolve(false);
          // MOD: block non-auth publishes when sync is off (allow only if type is heartbeat/auth related handled elsewhere)
          if (!this.isEnabled('gramlink_sync_enabled') && type !== 'auth') {
            return Promise.resolve(false);
          }
        ...
        value: function startHeartbeat(chatId, threadId) {
          this.stopHeartbeat();
          // MOD: require both heartbeat AND sync_enabled
          if (!this._connected || !this.isEnabled('gramlink_heartbeat') || !this.isEnabled('gramlink_sync_enabled')) return;
          var self = this;
          var myId = getDeviceId();
          var doBeat = function doBeat() {
            if (!self._connected) return;
            if (!self.isEnabled('gramlink_heartbeat') || !self.isEnabled('gramlink_sync_enabled')) {
              self.stopHeartbeat();
              return;
            }
    ...
    function initSettings() {
      ...
      SettingsApi.addParam({
        ...
        onChange: function onChange(value) {
          ...
          // MOD: fully stop network activity when sync is disabled
          try {
            var client = GramLinkClient.getInstance();
            if (!value) {
              client.stopHeartbeat();
              Lampa.Noty.show(Lampa.Lang.translate('gramlink_settings_sync_enabled') + ': OFF');
            } else {
              var channelId = Lampa.Storage.get('gramlink_channel_id', '');
              var syncLogTopicId = Lampa.Storage.get('gramlink_sync_log_topic', '');
              if (channelId && syncLogTopicId && client.isConnected() && Lampa.Storage.get('gramlink_heartbeat', false)) {
                client.startHeartbeat(channelId, syncLogTopicId);
              }
              Lampa.Noty.show(Lampa.Lang.translate('gramlink_settings_sync_enabled') + ': ON');
            }
          } catch (e) {}
        }
      });
    ...
    function addMenu() {
      function insert() {
        ...
        // MOD: Quick Sync ON/OFF toggle in side menu
        function syncLabel() {
          var on = Lampa.Storage.get('gramlink_sync_enabled', false) === true || Lampa.Storage.get('gramlink_sync_enabled', false) === 'true';
          return on ? Lampa.Lang.translate('gramlink_menu_sync_on') : Lampa.Lang.translate('gramlink_menu_sync_off');
        }
        var toggleBtn = $("<li class=\"menu__item selector gramlink-sync-toggle\">\n            <div class=\"menu__ico\">\n                <svg viewBox=\"0 0 24 24\" fill=\"currentColor\" xmlns=\"http://www.w3.org/2000/svg\">\n                    <path d=\"M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z\"/>\n                </svg>\n            </div>\n            <div class=\"menu__text\">" + syncLabel() + "</div>\n        </li>");
        toggleBtn.on('hover:enter', function () {
          var cur = Lampa.Storage.get('gramlink_sync_enabled', false);
          var next = !(cur === true || cur === 'true');
          Lampa.Storage.set('gramlink_sync_enabled', next);
          try {
            var client = GramLinkClient.getInstance();
            if (!next) {
              client.stopHeartbeat();
              Lampa.Noty.show(Lampa.Lang.translate('gramlink_sync_toggled_off'));
            } else {
              var channelId = Lampa.Storage.get('gramlink_channel_id', '');
              var syncLogTopicId = Lampa.Storage.get('gramlink_sync_log_topic', '');
              if (channelId && syncLogTopicId && client.isConnected() && (Lampa.Storage.get('gramlink_heartbeat', false) === true || Lampa.Storage.get('gramlink_heartbeat', false) === 'true')) {
                client.startHeartbeat(channelId, syncLogTopicId);
              }
              Lampa.Noty.show(Lampa.Lang.translate('gramlink_sync_toggled_on'));
            }
          } catch (e) {}
          toggleBtn.find('.menu__text').text(syncLabel());
        });
        $('.menu .menu__list').eq(0).append(toggleBtn);
      }
